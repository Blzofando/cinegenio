...
const ChallengeDetailsModal: React.FC<ChallengeDetailsModalProps> = ({ item, isCompleted, onClose, onComplete, onAddToWatchlist, isInWatchlist }) => {
    ...
    const watchProviders = details?.['watch/providers']?.results?.BR;
        ...
        <div className="flex flex-col items-center p-4 text-center">
            ...
            {selectedItem && (
                <ChallengeDetailsModal
                    item={selectedItem.step}
                    isCompleted={selectedItem.step.completed}
                    onClose={() => setSelectedItem(null)}
                    onComplete={() => handleToggleStep(selectedItem.index)}
                    onAddToWatchlist={() => handleAddToWatchlist(selectedItem.step)}
                    isInWatchlist={isInWatchlist(selectedItem.step.tmdbId)}
                />
            )}
