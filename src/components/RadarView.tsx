...
import { RadarItem, WatchlistItem, DisplayableItem } from '../types'; // 1. TIPO ADICIONADO
...
// 3. COMPONENTES LOCAIS REMOVIDOS (Modal, WatchProvidersDisplay, DetailsModal)
...
const RadarView: React.FC = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<DisplayableItem | null>(null); // 4. ESTADO ATUALIZADO
    ...
    useEffect(() => {
        ...
        const unsubRelevant = onSnapshot(relevantReleasesCollection, (snapshot) => {
            ...
            if (isRelevantLoading) setIsRelevantLoading(false);
        });
    ...
    // 5. NOVA FUNÇÃO PARA RENDERIZAR OS BOTÕES DE AÇÃO
